...
#================================================================
#% SYNOPSIS
#+    script usage: install_cli.sh [<flags>]
#%
#% DESCRIPTION
...
#%
#% EXAMPLES
#%    ./install_cli.sh -g -u arg1 -p arg2 -r arg3 -o arg4
#%
#================================================================
#- IMPLEMENTATION
#-    version         install_cli.sh (www.trycarbonara.com) 0.0.2
...
#================================================================
#  HISTORY
#     2023/06/09 : saurabh-carbonara : Script creation
# 
#================================================================
#  DEBUG OPTION
...
if [ "$cvalue" == true ] ; then
  ...
  if [ "$gvalue" == true ] ; then
    ...
    if [ "$yvalue" == false ] ; then
      read -p "Confirm updating GPU driver and reboot (y/n)?" choice
      case "$choice" in 
        y|Y ) fvalue=true;;
        n|N ) fvalue=false;;
        * ) fvalue=false;;
    ...
    if [ "$fvalue" == true ] || [ -x "$(command -v nvidia-smi)" ] ; then
  ...
  sudo curl -fsSL https://raw.githubusercontent.com/TryCarbonara/NodeInstallation/main/client/node-exporter/node_exporter.service -o /etc/systemd/system/node_exporter.service \
    && sudo mkdir -p /etc/sysconfig \
    && sudo echo 'OPTIONS="--collector.textfile.directory /var/lib/node_exporter/textfile_collector --collector.processes --collector.systemd --collector.tcpstat --collector.cpu.info --collector.diskstats.ignored-devices=\"^(ram|loop|fd)\\\\d+$\"  --web.listen-address=:'$nvalue'"' | sudo tee /etc/sysconfig/node_exporter > /dev/null
  ...
  if [ $lvalue == false ] ; then
    ...
    if [ -z "$evalue" ]; then
      ...
      sudo echo "$evalue" | sudo tee -a /etc/default/grafana-agent > /dev/null
    fi
